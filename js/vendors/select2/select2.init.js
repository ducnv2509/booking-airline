...
$(document).ready(function () {
  ...
  $('.select-2:not([multiple])').select2({
    ...
    if ($(window).width() >= 789) {
      if ($(this).attr('data-pos') == 'right') {
        var containerWidth = $(this).siblings('.select2-container').width()
        $('.select2-container .select2-dropdown').css('right', 0 - containerWidth);
        $('.select2-container .select2-dropdown').css('left', 'auto')
      }
    ...
    if ($(window).width() <= 767) {
      ...
      $('body > .select2-container .select2-dropdown').prepend('<div class="close-select h3"><div class="close-select__btn" href="javascript:void(0)" onclick="closeSelect()"><img src="media/icons-color/subdefault/default/24x24-close.svg" alt="" class="ic-default"></div>' + x + '</div>');
  ...
  function templateImgRes (img) {
    ...
    if (img.element.attributes.dataimgClass) {
      ...
      return $img = $('<div class="select-tpl-img-wrap"><div class="row row-10 align-items-center"><div class="col"><div class="select-tpl-txt">' + img.text + '</div><div class="select-tpl-txt-sub h7 color-default-3">' + img.element.attributes.datasub.value + '</div></div></div></div>');
    }
  };
